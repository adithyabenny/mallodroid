...
def _returns_true(_method):
    ...
    if len(_instructions) == 2:
        _i = "->".join([_instructions[0].get_output(),
...
def _check_trust_manager(_method, _vm, _vmx):
    ...
    if _has_signature(_method, [_check_server_trusted]):
        ...
        if _class_implements_interface(_class, _trustmanager_interfaces):
            ...
            _verification = False
            if not _empty:
                _verification = _contains_conditional_syntax(
                    _method) or _invoke_method(_method)
            _custom_trust_manager.append({'class': _class,
                                          'xref': _xref,
                                          'java_b64': _java_b64,
                                          'empty': _empty,
...
def _check_hostname_verifier(_method, _vm, _vmx):
    ...
    if _has_signature(_method,
        ...
        if _class_implements_interface(
                _class, _verifier_interfaces) or _class_extends_class(
                _class, _verifier_classes):
            _java_b64, _xref = _get_javab64_xref(_class, _vmx)
            _empty = _returns_true(_method) or _returns_void(_method)
            _verification = False
            if not _empty:
...
def _check_all(_vm, _vmx, _gx):
    ...
    return {'trustmanager': _custom_trust_manager,
...
def _store_java(_vm, _args):
    ...
    for _class in _vm.get_classes():
        try:
            ...
            print(
                "Could not process {:s}: {:s}".format(
                    _class.get_name(), str(e)))
